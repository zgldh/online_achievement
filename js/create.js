$(function()
{
    ...
    step_2_to_3_btn.click(function()
    {
        setStep(3);
        // 设置预览步骤
        procedure_editor.find('textarea').each(function()
        {
            var textarea = $(this);
        ...
        $('#ach_procedure').val($.toJSON(procedure));
    ...
    ach_categories.select2(
    {
        ...
        multiple : true,
        formatResult : ach_categories_format,
        formatSelection : ach_categories_format,
        formatInputTooShort : function(term, minLength)
        {
            return '请至少输入' + minLength + '个字';
        },
    ...
    var ach_uploaded_id = $('#ach_uploaded_id');
    ...
    image_file
        .change(function()
        {
            ...
            var iframe = $('<iframe style="display:none;" name="' + iframe_name
                + '" id="' + iframe_name + '"></iframe>');
            $(document.body).append(iframe);
            var action_url = '/create/jsonp_logo_upload?callback=LOGO_CALLBACK&iframe_id='
                + iframe_name;
            logo_form.attr('action', action_url).attr('target', iframe_name)
                .submit();
            image_file.hide();
    ...
    logo_confirm_btn.click(function()
    {
        ...
        ach_logo_crop.val($.toJSON(crop));
        ach_uploaded_id.val(image.uploaded_id);
        // 设置预览
...
var LOGO_CALLBACK = function(re, iframe_id)
{
    if (typeof (re.error_msg) == 'undefined')
    {
        ...
        logo_img.attr('src', re.image_url).css('width', re.image_width + 'px')
            .css('height', re.image_height + 'px');
