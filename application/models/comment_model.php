...
class Comment_model extends MY_Model
{
    ...
    {
        $raw = $this->db->get_where ( Comment_model::TABLE, array (
                CommentPeer::PK => $comment_id 
        ) )->row_array ();
     ...
     * @return multitype:CommentPeer
     */
    public function getByReplyCommentId($reply_comment_id, $offset = 0, $limit = null)
    {
        $this->db->where ( 'reply_comment_id', $reply_comment_id );
        $this->db->order_by ( 'comment_id', 'DESC' );
        $limit_obj = new DB_Limit ( $limit, $offset );
        $limit_obj->setLimit ( $this->db );
        $result = $this->db->get ( Comment_model::TABLE )->result_array ();
...
class CommentPeer extends BasePeer
{
    ...
    public static function create($content, $user_id, $achievement_id = null, $track_id = null, $reply_comment_id = null)
    {
        ...
        $comment->achievement_id = ( int ) $achievement_id;
        $comment->track_id = ( int ) $track_id;
        $comment->reply_comment_id = ( int ) $reply_comment_id;
        return $comment;
    }
    ...
    public function getTrack()
    {
        ...
        $track = TrackPeer::model ()->getByPK ( $this->track_id );
        return $track;
    }
    ...
    {
        $comment = CommentPeer::model ()->getByPK ( $this->reply_comment_id );
        return $comment;
    }
     ...
     * @return multitype:CommentPeer
     */
    public function getChildrenComments($offset = 0, $limit = null)
    {
        $comments = CommentPeer::model ()->getByReplyCommentId ( $this->comment_id, $offset, $limit );
        return $comments;
    }
}
